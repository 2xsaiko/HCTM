...
class Processor : IProcessor {
  ...
  override fun saveData(tag: QNBTCompound) {
    ...
    tag.ushort["rp"] = rp
    tag.ushort["pc"] = pc
    tag.ushort["ra"] = resetAddr
    tag.ushort["ba"] = brkAddr
    tag.short["f"] = packShort(*flags)
    tag.bool["t"] = timeout
  ...
  override fun loadData(tag: QNBTCompound) {
    ...
    rp = tag.ushort["rp"]
    pc = tag.ushort["pc"]
    resetAddr = tag.ushort["ra"]
    brkAddr = tag.ushort["ba"]
    unpack(tag.short["f"]).copyTo(flags)
    timeout = tag.bool["t"]
  ...
  private fun peek1(addr: Int): Int {
    ...
    if (busEnabled && uaddr in ubusOff..ubusOff + 0x00FF) {
      val bus = mem.bus()
      if (bus == null || bus.busId != mem.targetBus) {
        timeout = true
        return 0
      } else return bus.peek((addr - ubusOff).toByte()).unsigned
    } else return mem[addr.toShort()].unsigned
  }
  ...
  private fun poke1(addr: Int, b: Int) {
    ...
    if (busEnabled && uaddr in ubusOff..ubusOff + 0x00FF) {
      val bus = mem.bus()
      if (bus == null || bus.busId != mem.targetBus) timeout = true
      else bus.poke((addr - ubusOff).toByte(), b.toByte())
  ...
  private fun popM(): Int {
    if (flags[M]) return pop1()
    else return pop2()
  }

  private fun popX(): Int {
    if (flags[X]) return pop1()
    else return pop2()
  }
    ...
    fun mmu(data: Int) {
      when (ub(data)) {
        0x00 -> {
          ...
          if (mem.isBusConnected && mem.bus()!!.busId != mem.targetBus) timeout = true
        }
   ...
   */
  fun sb(i: Int): Int = i.toByte().toInt()

  /**
   * Convert to unsigned byte
   */
  fun ub(i: Int): Int = i.toByte().unsigned

  /**
   * Convert to signed short
   */
  fun ss(i: Int): Int = i.toShort().toInt()

  /**
   * Convert to unsigned short
   */
  fun us(i: Int): Int = i.toShort().unsigned

  fun sM(i: Int): Int = if (flags[M]) sb(i) else ss(i)
  fun sX(i: Int): Int = if (flags[X]) sb(i) else ss(i)
  fun uM(i: Int): Int = if (flags[M]) ub(i) else us(i)
  fun uX(i: Int): Int = if (flags[X]) ub(i) else us(i)
