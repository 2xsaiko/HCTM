 ...
 */
fun <E : PartExt<out Any?>, T> connectionHandler(op: ConnectionHandlerScope<E, T>.() -> Unit): ConnectionHandler<E> {
  val sc = ConnectionHandlerScopeImpl<E, T>().also(op)
  return ConnectionHandlerImpl(sc.parts)
}

interface ConnectionHandler<E : PartExt<out Any?>> {
  fun tryConnect(self: NetNode, world: ServerWorld, pos: BlockPos, nv: NodeView): Set<NetNode>
}

private class ConnectionHandlerImpl<E : PartExt<out Any?>, T> constructor(private val parts: List<ConnectionHandlerPart<E, T>>) : ConnectionHandler<E> {
...
private abstract class ConnectionHandlerPart<E : PartExt<out Any?>, T> {
...
interface ConnectionHandlerScope<E : PartExt<out Any?>, T> {
  fun connectionRule(op: PartConfigScope<E, T>.() -> Unit)
}

interface PartConfigScope<E : PartExt<out Any?>, T> {
  fun forOutputs(op: OutputsScope<E>.() -> List<T>)
  fun connect(op: ConnectScope<E, T>.() -> NetNode?)
}

interface OutputsScope<E : PartExt<out Any?>> {
  val self: TNetNode<E>
  val world: ServerWorld
  val pos: BlockPos
}

interface ConnectScope<E : PartExt<out Any?>, out T> {
  val output: T
  val self: TNetNode<E>
  val world: ServerWorld
  val pos: BlockPos
...
private class ConnectionHandlerScopeImpl<E : PartExt<out Any?>, T> : ConnectionHandlerScope<E, T> {
...
private class PartConfigScopeImpl<E : PartExt<out Any?>, T> : PartConfigScope<E, T> {
...
private class OutputsScopeImpl<E : PartExt<out Any?>>(
  override val self: TNetNode<E>,
  override val world: ServerWorld,
  override val pos: BlockPos
) : OutputsScope<E>

private class ConnectScopeImpl<E : PartExt<out Any?>, out T>(
